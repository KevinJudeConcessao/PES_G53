...
/// [clang] compiler-diagnostics-statement
///
///       labeled-statement:
...
///
/// [clang] compiler-diagnostics-statement:
///             clang-compiler-error-statement
///             clang-compiler-warning-statement
StmtResult
...
Retry:
  ...
  case tok::kw___clang_compiler_error: // Clang extension: __clang_compiler_error
    ...
    break;
  case tok::kw___clang_compiler_warning:  // Clang extension: __clang_compiler_warning
    Res = ParseClangDiagnosticsStatement();
    SemiError = "__clang_compiler_warning";
    break;
...
StmtResult
Parser::ParseClangDiagnosticsStatement() {
    ...
    if (Tracker.expectAndConsume(diag::err_expected_lparen_after, ""))
        return StmtError();
    ExprResult StrResult = ParseStringLiteralExpression();
    Tracker.consumeClose();
    Expr* ExprPtr = ExprRes.get();
    llvm::APSInt Integer;
    if(ExprPtr->EvaluateAsInt(Integer, Actions.Context) && !Integer.isNullValue()) {
        StringLiteral* Diagnostics = StrResult.getAs<StringLiteral>();
        llvm::StringRef Str = Diagnostics->getString();
        DiagnosticIDs::Level DiagLevel = DiagnosticIDs::Level::Error;
        StmtResult Result = StmtError();
        if (Optok.is(tok::kw___clang_compiler_warning)) {
            DiagLevel = DiagnosticIDs::Level::Warning;
            Result = StmtEmpty();
        }
        const unsigned DiagID = Diags.getDiagnosticIDs()->getCustomDiagID(DiagLevel, Str);
        Diag(SourceLocation(), DiagID);
    }
