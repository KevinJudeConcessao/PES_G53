...
NamespaceDecl *Sema::lookupStdReflectionNamespace() {
    if (!StdReflectionNamespace) {
        if (auto Std = getStdNamespace()) {
            LookupResult Result(*this, &PP.getIdentifierTable().get("reflection"),
                                SourceLocation(), LookupNamespaceName);
            if (!LookupQualifiedName(Result, Std) || !(StdReflectionNamespace = Result.getAsSingle<NamespaceDecl>())) {
                Result.suppressDiagnostics();
            }
        }
    }
    return StdReflectionNamespace;
}
